...
module.exports = client => {
    ...
    client.on("interactionCreate", async (interaction) => {
        ...
        if (temptype == "close") {
            ...
            let button_ticket_verify = new MessageButton().setStyle('SUCCESS').setCustomId('ticket_verify').setLabel("Verify this Step").setEmoji("832687320465014834")
            channel.send({
                content: `<@${buttonuser.id}>`,
                embeds: [new Discord.MessageEmbed()
                ...
                collector.on('end', collected => {
                    if (!edited) {
                        ...
                        msg.edit({
                            content: `<@${buttonuser.id}>`,
                            embeds: [endedembed],
                            components: [new MessageActionRow().addComponents(button_ticket_verify.setDisabled(true).setLabel("FAILED TO VERIFY").setEmoji("904319985004978198").setStyle('DANGER'))]
                        }).catch((e) => {
                            console.log(String(e).grey)
                        });
                    }
        ...
        if(interaction?.guildId && interaction?.isSelectMenu() && interaction?.message && interaction?.message.author.id == client.user.id){
            ...
            if(message.id == settings.messageId && (channelId == settings.channelId || message.channelId == settings.channelId)){
               let index = settings.data.findIndex(v => v.value == values[0]);
                ...
                await interaction?.editReply({content: `<:yes:946018402273726464> **Your Ticket is created!** <#${ch.id}>`, ephemeral: true});
                })
            }
        }
    })
